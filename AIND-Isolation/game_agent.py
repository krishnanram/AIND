...
L_CENTRAL_DISTANCE = .333
H_CENTRAL_DISTANCE = .667
...
def custom_score(game, player):
    ...
    # increase overlapping towards end game
    overlapping = 1.
    ...
    return player_moves - (overlapping * opponent_moves)
...
def custom_score_2(game, player):
    ...
    return len(player_moves) - len(opp_moves) + progressive_bonus
...
def custom_score_3(game, player):
    ...
    distanceToCentral = float(x) / game.width
    y_centralness = float(y) / game.height

    if L_CENTRAL_DISTANCE <= distanceToCentral or distanceToCentral <= H_CENTRAL_DISTANCE:
      position_bonus += .5

    if L_CENTRAL_DISTANCE <= y_centralness or y_centralness <= H_CENTRAL_DISTANCE:
      position_bonus += .5
    ...
    if L_CENTRAL_DISTANCE > x_opp_centralness or x_opp_centralness > H_CENTRAL_DISTANCE:
      position_bonus += .5
    if L_CENTRAL_DISTANCE > y_opp_centralness or y_opp_centralness > H_CENTRAL_DISTANCE:
      position_bonus += .5
